...
serve(async (req) => {
  ...
  try {
    ...
    const { action, provider, mode, api_key, webhook_secret, is_active } = body;

    if (action === "status") {
      const { data } = await admin.from("payment_settings").select("id, provider, mode, is_active, api_key, webhook_secret").order("created_at");
    ...
    if (is_active) {
      await admin.from("payment_settings").update({ is_active: false }).neq("provider", provider);
    }

    const { error } = await admin.from("payment_settings").upsert(row, { onConflict: "provider" });
