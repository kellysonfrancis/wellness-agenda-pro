...
import { mockPayments, getClientName, getAccountName } from "@/data/mockData";
                ...
                <tbody className="divide-y divide-border">
                  {pagos.map(p => (
                    <tr key={p.id} className="hover:bg-muted/30">
                      ...
                      <td className="p-4">R$ {p.valorPago.toLocaleString("pt-BR")}</td>
                      <td className="p-4 capitalize">{p.metodo}</td>
                      <td className="p-4">{p.contaDestinoId ? getAccountName(p.contaDestinoId) : <span className="text-muted-foreground">—</span>}</td>
